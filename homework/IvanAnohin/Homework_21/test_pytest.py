...
def new_post(name="First object", data=None):
    ...
    allure.attach(str(obj_id), name="Created object ID", attachment_type=allure.attachment_type.TEXT)
    return obj_id
...
@pytest.fixture
def obj_fixture():
    with allure.step("Фикстура: создать объект для теста"):
...
@allure.title("Создание объекта с параметрами: {name}, {data}")
